...
impl RequestSerializer {
    pub fn new(connection: Connection<TcpStream>, method: Method, entry_point: EntryPoint) -> Self {
        RequestSerializer {
            temp_url: Url::parse("http://localhost/").unwrap(), // TODO
...
impl<'a> ser::Serializer for &'a mut RequestSerializer {
    type Ok = ();
    type Error = Error;
...
impl<'a> ser::SerializeStruct for &'a mut RequestSerializer {
    type Ok = ();
    type Error = Error;
        ...
        where T: ?Sized + Serialize
    {
        match key {
            "path" => {
                ...
                {
                    let mut serializer =
                        UrlPathSerializer::new(&self.entry_point,
                                               self.temp_url.path_segments_mut().unwrap());
...
impl<'a> ser::SerializeStructVariant for &'a mut RequestSerializer {
    type Ok = ();
    type Error = Error;
    fn serialize_field<T>(&mut self, key: &'static str, value: &T) -> Result<()>
        where T: ?Sized + Serialize
